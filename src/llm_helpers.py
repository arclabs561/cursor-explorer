...
def require_client() -> Any:
	# Always load .env early so downstream calls see keys
	try:
		envmod.load_dotenv_if_present()
	except Exception:
		pass
	if OpenAI is None:
		raise LLMNotAvailable("OpenAI client not installed. Install with: uv sync --extra llm")
	api_key = os.getenv("OPENAI_API_KEY")
	if not api_key:
		# Final fallback attempt already handled via envmod; re-check
		api_key = os.getenv("OPENAI_API_KEY")
		if not api_key:
			raise LLMNotAvailable("OPENAI_API_KEY not set")
	# The OpenAI client reads from env by default; pass nothing to keep it simple
	return OpenAI()
